...
def step_decay(epoch,initial_lrate=0.00001):
   ...
   return lr
def train():
    dataset = get_input('data/train_input_bert_final.pkl')
    train=dataset
    ...
    for i, (tra_index, val_index) in enumerate(kfold.split(train[0])):
        ...
        filepath_f1 = "model/ED_binary_model_bert_f1.h5_" + str(i)
...
def predict_f1(input_file,out_file,model_index=0):
    ...
    for i in range(5):
        if not i==model_index:
            continue
        print(i)
        filepath_loss = "model/ED_binary_model_bert_f1.h5_" + str(i)
        model = link_model()
        model.load_weights(filepath_loss)
        with open(input_file,'r') as f:
            for j,line in enumerate(f):
